...
#
# Только .pk3, и это важно: в списке ioquake3 есть ещё baseq3/vm/*.qvm, а в
# папке игрока их нет — они лежат внутри паков. Помеченные «своими», они
# считались недостающими всегда, и клиент просил указать папку при каждом
# заходе, сколько бы раз её ни указывали. Из списка их убираем совсем:
# движок берёт эти модули из паков.
baseq3 = c.get("baseq3", {})
baseq3["files"] = [f for f in baseq3.get("files", []) if f["src"].endswith(".pk3")]
for f in baseq3["files"]:
    f["own"] = True
